...
from . import common_ln
# from models import common_ln
...
class ImagePolicyModelSS(common_ln.ResnetBase):
    def __init__(self, backbone, warp=False, pretrained=False, all_branch=False, **kwargs):
        ...
        self.warp = warp
        self.rgb_transform = common_ln.NormalizeV2(
            mean=[0.485, 0.456, 0.406],
            std=[0.229, 0.224, 0.225]
        )

        self.deconv = nn.Sequential(
            nn.LayerNorm(self.c + 128),
        ...
        self.location_pred = nn.ModuleList([
            nn.Sequential(
                ...
                common_ln.SpatialSoftmax(ow, oh, STEPS),
            ) for i in range(3)
        ])
    ...
    def forward(self, image, command=None, velocity=None):
        ...
        location_pred = common_ln.select_branch(location_preds, command)
        ...
        location3d_pred = common_ln.select_branch(location3d_preds, command)
